...
## 文脈
`submon_flt` の DDD 移行タスク。前提: `00-analysis.md` / `01-roadmap.md`、**T08 完了済み**
（memorize_card が feature 化されていること。EventBus の主要利用元のため）。

`lib/events.dart` の `EventBus` が `main.dart` / `pages/home_page.dart` /
`pages/home_tabs/tab_memorize_card.dart` 間でグローバルにイベント配信しており、
Riverpod のデータフローと二重化している（問題 P7）。状態の出所が追えない。
...
- 各イベントを、対応する feature の Notifier の状態変化、または Riverpod の
  ...
  - 例: カード更新通知 → memorize_card feature の Notifier 状態を購読。
